...
- 📊 **实时统计信息** – 显示明文 UTF-8 字节数，以及密文长度（根据当前输出格式：Base64 字符数或十六进制字节数）
...
3. **解密**  
   - 在“密文”区域粘贴待解密的字符串（支持 Base64 或 Hex，程序自动识别）。  
   - 确保密钥正确。  
   - 点击 “← 解密”，成功解密后明文会显示并自动复制。

4. **实时统计**  
   - 明文区域下方显示当前文本的 UTF-8 字节数。  
   - 密文区域下方显示当前密文的长度：Base64 格式显示字符数，十六进制格式显示字节数。若密文无效则提示。
...
- v1.0 (2026-06) – 首个正式版本，支持基础加密/解密、中英文界面、剪贴板联动及实时统计。
...
本项目提供的 **Python 客户端** 与 **Java 客户端（https://github.com/BI3BJU/Text-Encryption-Tool-Java）** 拥有完全一致且对齐的底层加密架构。
